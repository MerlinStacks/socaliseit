 ...
 * Why: Extracted from page.tsx so the page can be a server component that
 * prefetches data. This client component receives initialData from the server
 * and passes it to useCalendarOrchestration, so React Query renders with
 * real data on the FIRST frame — no API waterfall.
 */
...
import type { CalendarPost, CalendarNote } from '@/components/calendar/calendar-types';
...
export function CalendarClient({ initialData }: CalendarClientProps) {
    ...
    const cal = useCalendarOrchestration(initialData);
